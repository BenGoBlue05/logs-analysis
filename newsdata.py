...
def days_with_errors_over_1_percent():
    query = '''
        SELECT day, rate
        FROM(
            SELECT to_char(time, 'FMMonth DD, yyyy') AS day, avg(CASE WHEN status like '40%' THEN 1 ELSE 0 END) as rate
            FROM log
            GROUP BY day
        ) as subq
        WHERE rate > 0.01
...
def print_result():
    print("\n{0}\n\n{1}\n\n{2}\n".format(most_popular_articles(), most_popular_authors(),
                                         days_with_errors_over_1_percent()))
